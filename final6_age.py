...
model1.add(Flatten())
model1.add(Dense(units*2, name="my_convdense", trainable=False))
model1.add(BatchNormalization())
#model.add(act)
model1.add(Activation('relu'))
model1.add(Dropout(dropout))
...
model5 = Sequential()
model5.add(LSTM(300, input_shape=(500,300,),name="my_lstmnew", trainable=False))
#model1.summary()
#model3.summary()
#model5.summary()
'''
...
model5.add(Dropout(dropout))
'''
model1.load_weights('5_age.h5', by_name=True)
model3.load_weights('5_age.h5', by_name=True)
model5.load_weights('5_age.h5', by_name=True)
#model5.load_weights('4.h5', by_name=True)
...
model4.add(Activation('relu'))
model4.add(Dropout(dropout))
model4.add(Dense(3,  name="my_dense_5", trainable = False))
model4.add(BatchNormalization())
model4.add(Activation('relu'))
model4.add(Dropout(dropout))


#for lyr in model4.layers:
	#print (lyr.output_shape)
model4.load_weights('final3_age.h5')
...
intermediate_layer_model = Model(inputs=model4.input,
                                 outputs=model4.get_layer("my_dense_3").output)
...
max_post_len_lstm = 500
corpus_post_lstm = pad_sequences(corpus_post_word_sequences,
                              maxlen = max_post_len_lstm)
...
model5n = Sequential()
model5n.add(Embedding(nb_words + 1,
                     embedding_dim,
                     weights = [word_embedding_matrix],
                     input_length = max_post_len_lstm,
                     trainable = False))
#model3n.summary()
#model3.add(TimeDistributed(Dense(embedding_dim, name="my_timedense", trainable=False)))
model5n.add(model5)
model5n.add(Dropout(dropout))
model5n.summary()

modelnn = Sequential()
modelnn.add(Merge([model1n, model5n, model3n], mode='concat'))
#modelnn.add(model5)
...
#modelnn.add(Activation('relu'))
#modelnn.add(Dropout(dropout))
modelnn.load_weights('5_age.h5', by_name=True)

#for lyr in modelnn.layers:
#        print (lyr.output_shape)
modelnn.summary()

#print ("model4 output shape" + str(model4.layers[-1].output_shape))
#model4.outputs = [model4.layers[-1].output]

#intermediate_output = intermediate_layer_model.predict(data)

modelfn = Sequential()
modelfn.add(intermediate_layer_model)
modelfn.summary()

model = Sequential()
#model.add(Merge([model1, model2, model3, model4], mode='concat'))
model.add(Merge([modelnn, modelfn], mode='concat'))
...
t0 = time.time()
save_best_weights = 'final6_age.h5'
...
test_corpus_post_lstm = corpus_post_lstm[int(len(corpus_post_lstm) * 0.9):]
corpus_post_lstm = corpus_post_lstm[:int(len(corpus_post_lstm) * 0.9)]
...
history = model.fit([corpus_post, corpus_post_lstm,  corpus_post, f_tr],
                    y_train,
                    batch_size=8,
...
score = model.evaluate([test_corpus_post, test_corpus_post_lstm, test_corpus_post,  f_te], test_y ,batch_size=4, verbose=0)
