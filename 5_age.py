...
model1.add(Flatten())
model1.add(Dense(units*2, kernel_initializer=weights, bias_initializer=bias, name="my_convdense"))
model1.add(BatchNormalization())
#model.add(act)
model1.add(Activation('relu'))
model1.add(Dropout(dropout))
...
model2.add(LSTM(embedding_dim, name="my_lstmnew"))
...
model.add(Dense(3, kernel_initializer=weights, bias_initializer=bias, name="my_endense3"))
model.add(BatchNormalization())
...
history = model.fit([corpus_post, corpus_post_lstm, corpus_post],
...
score = model.evaluate([test_corpus_post, test_corpus_post_lstm, test_corpus_post], test_y, verbose=0)
